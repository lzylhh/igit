...
path = "C:\\Users\\dell\\Desktop\\googletest"#修改包路径即可，该版本适配windows
...
def get_all_commit(repo_name):
	...
	for line in os.popen(all_commits).readlines():
		result.append(str(line[:-1]))
	return result
def get_delta_data(name,obj_size , offset_in_packfile, base_hash = "*"):
	#***首先读取所有的数据
	f = open(name, 'rb')
...
def delta_obj_data(delta_data,neg_offset, obj_type):

	head = 0x00 ^ (obj_type << 4)
	l = len(delta_data) 
	MSB = libgit.MSB_len(l)
		...
		for fn in files:
			...
			if fn.endswith(".idx"):
				...
				for line in lines:
					...
					if len(one) == 5:
						_data = libgit.obj_data(repo.get(one[0]))
						...
						compressed_delta_data = get_delta_data(file_name[:-4] + ".pack", size_in_packfile, offset_in_packfile, one[6])
						# delta_data = del_instruction(compressed_delta_data)
						# _data = delta_obj_data(zlib.decompress(compressed_delta_data[0]), guangbiao - offset_ji[one[6]], 6)
						_data = delta_obj_data(zlib.decompress(compressed_delta_data[0]), compressed_delta_data[2], 6)
						pack_file.write(_data)
						sha1.update(_data)
						offset_ji[one[0]] = guangbiao
						guangbiao += len(_data)
